...
* `options <Object>`, со свойствами:
    * `logger <Logging>`  
    объект, реализующий интерфес Logging, позволяет заменить стандартный способ вывода сообщений
...
### addTest(test[, options])
  ...
  * `options <Object>`, со свойствами:
      ...
      * `name <string>` 
      Имя теста, по умолчанию используется имя функии test
...
Показывет, была ли подготовителная операция.

### getHasClean()

* Возвращает `<bool>`

Показывет, была ли заключительная операция.

### getIsMainTestFailed()

* Возвращает `<bool>`

Показывет, завершился ли ошибкой основной тест.

### getIsPrepareFailed()

* Возвращает `<bool>`

Показывет, завершилась ли ошибкой подготовителная операция.

### getIsCleanFailed()

* возвращает `<bool>`

Показывет, завершилась ли ошибкой заключительная операция.
...
Показывет, завершился ли тест успешно. Если была ошибка при выполнении подготовительной опрации, теста или закличительной
...
Если все тесты прошли успешно, свойство failed будет остутсвовать.
...
Обработчик события, вызов которого завершиля ошибкой.
